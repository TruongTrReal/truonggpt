...
import { type ChatCompletionOptions } from "https://deno.land/x/openai@1.4.2/mod.ts";
...
export function getChatStream(
  options: ChatOptions,
  apiKey: string,
  model: "gpt-4-0613" | "gpt-3.5-turbo-16k" = "gpt-3.5-turbo-16k"
): ReadableStream {
...
export function sanitizeMessages(
  ...
  // Ensure the conversation history is not too long
  const trimmedMessages = messages.slice(-historyLength);
  
  console.log("filteredMessages", trimmedMessages);
  // Return the sanitized messages
  return trimmedMessages;
}

  // old code work with old library
//   return messages.slice(-historyLength).map(({ content, role }) => {
//     if (role !== "assistant" && role !== "user" && role !== "function" && role !== "system") {
//       return { role: "", content: "" };
//     }
